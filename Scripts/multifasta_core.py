...
def write_bed(subset_df, output_file, chromosome):
    """Write positions passing filter in bed format."""
    with open(output_file, 'w') as f: 
        for col in subset_df.columns: 
            f.write(chromosome + "\t" + str(col) + "\t" + str(col + 1))
            f.write("\n")
    f.close()
...
def main(): 
    ...
    parser.add_argument('perc_deletion', help = "Percent deletion for filtering multifasta (e.g. 0.95 for 95%% partial deletion).")
    parser.add_argument('chromosome', help = "Name of bacterial chromosome for bed file output.")
    ...
    subset_df = percent_deletion(df, args.perc_deletion)
    print ("Partial deletion filtering complete. Outputting fasta and bed files.")
    write_multifasta(subset_df, args.output_dir + '/' + args.output + '.fasta')
    write_bed(subset_df, args.output_dir + '/' + args.output + '.bed', args.chromosome)
    print ("Done!")
